...
*(Phase 11, 2026-09-13: the manual correction on top of it exists — both facets are controls on the Attributes tab's **Correct** section (§9.5), each protected from the next recompute by its own flag and each with a way back to the machine's call, and the curated tag layer of §8 is built. The taxonomy question itself stays open; the correction path is what §4 said it would be.)*
...
tags, sample_tags                                   -- CURATED user-owned tags, samples only
  -- Built Phase 11 (2026-09-13, schema v11): tags (id, name UNIQUE COLLATE NOCASE) and
  -- sample_tags (sample_id FK, tag_id FK, added_at). No stage writes here; a tag no sample
  -- carries any more is forgotten. The list's "My tags" column and its text filter read it.
...
classification                                      -- samples only
  ...
  content_class_confirmed, structural_type_confirmed,   -- Phase 11 (v11): one protection flag PER
                                                    -- FACET — node E leaves a confirmed class, re-analysis
                                                    -- a confirmed type; the other facet stays the machine's
...
*(2026-09-10, the user's steer — the beta is usable but "needs much sleeker UI", and a design system of its own. Studied before building: `scripts/design_studies.py` renders the real window against a real index under a candidate **token set**, and `.docs/design_studies.md` records the diagnosis, the three directions and the choice. **Direction A, "instrument panel"** — near-black ground, hairlines instead of boxes, uppercase micro-labels, one restrained accent reserved for state, no alternating rows — chosen on density: in the same 250 px the current theme and A both fit ten list rows, the calmer direction seven, and A additionally pulls the Key column into view. The studies also fixed the shape of the work: a token layer alone leaves the transport row equally ragged in all four renders and the Attributes panel and tag bars unchanged, so the system is four layers — tokens (`design.py`, with `theme.py` derived from it and keeping every constant it exports today), components, a shared painting kit, and a gallery of every token and component in every state. This is a polish track alongside §12, not a phase ahead of Phase 10.)*

*(Built the same day, layers 1 and 4: **`design.py`** holds direction A as tokens — a surface ramp `canvas → sunken → ground → panel → raised → overlay` wide enough that elevation is a step in value rather than the 1px outline that used to sit on every widget; ink, state and — held deliberately apart from the chrome — **data** colours, since `AMBER` previously meant both "segment" and "chrome accent"; one spacing, radius and type scale. `theme.py` still exports every constant the painted views import, but each is now a view onto a token, so `mapview.py`'s `_UNSCORED` — the app's last raw hex — is gone and there is exactly one place a colour is decided. **`crate --gallery`** opens every token and control state on one page: the look is iterated there rather than against a 110k-file index, it grabs offscreen (`scripts/design_studies.py --variant gallery`), and a test builds it so a token that breaks a control is visible. Two Qt limits found and recorded: `text-transform` reaches a `QLabel` but **not** the `QGroupBox::title` subcontrol — the old theme's title rule had carried one that never did anything — and Fusion is not observable after `setStyleSheet`, which wraps the style. Still to come: the painting kit, then the rest of the panels.)*

*(Layer 2 the same day — **`icons.py`** and **`widgets.py`**. Icons are painter paths, not glyphs: `icon(name, colour, size)` tints one of fifteen shapes to the caller's state, since ▶ ■ ▸ ▾ ⚓ ↳ could not be tinted, shifted metrics with whatever face resolved them, and drew boxes in a face that lacked them. `Toolbar` gives a control row groups divided by a rule and equalises widths *within* a group — so the **transport row** is now transport (▶ ■), the view toggle (Spectrum) and the segment edits (Save / Discard / Delete) rather than six self-sized buttons in one run, with *Delete segment* as `intent="danger"` (quiet at rest, coloured on hover — the icon carries the intent, the chrome does not) and *Discard* as `quiet`; it claims no minimum width, or equalising would bring back the splitter squeeze of 2026-09-07/-08. The **view switch** is one `SegmentedControl` instead of two `autoExclusive` buttons. The **Attributes tab's difference bars** are `Meter`s: a labelled track with its value at the right, an em dash for an axis the item lacks, instead of a full-width `QProgressBar` reading "n/a" — one implementation shared with the tag scores. Also `SectionHeader`, `Chip`, `StatusPill`, and `HelpText`, which folds the Recompute tab's explanatory paragraph behind a "?". The tag bars and the vector strip stay for later: both need the header's layout reworked, not a component swapped in.)*
...
- **⚓ Anchor**: pins the current sample/segment as the comparison reference — unlocks the Attributes tab's per-axis ranges (§9.5) and is a prerequisite for Recompute ranking (§9.6). *(Since 2026-09-08, on the user's steer — "the whole idea of ranking and anchoring feels overdone": the anchor is a ⚓ at the start of every list row, sub-hit rows included. One click anchors that sample or hit **and ranks the list against it at once**, with the weight bars as they are; the anchor sorts to the top, the Similarity column appears, the map colours by it. There is no separate Anchor button any more (the A key anchors the selected row); ✕ clears the anchor and changes nothing else — the ranked order stays. Ranking is one vectorised pass over the feature table — measured 25 ms for 4.5k samples + 29k segments — so it needs neither incremental loading nor a cache; only the table's first load after a recompute takes a moment (0.6 s here, seconds at library scale) and runs on a thread, the newest click winning. This is within §9.6's policy: an explicit click, and cheap. *2026-09-09, the user: anchoring locked the window.* Not the maths — the list: Qt's proxy sorted 32k rows (every sample with all its sections, since 2026-09-08) by calling back into Python for each comparison, and then opened 3k samples in one go. Now the model sorts its own rows by a key (milliseconds) and the proxy only filters, and the samples with a winning section open in slices between event-loop turns, top of the list first. A warm anchor click holds the GUI thread for ~0.2 s and the first click after a load for at most 0.17 s at a time; a sort of the 32k rows is 0.43 s. No precomputation of anchor pairs was needed.)*
...
*(2026-09-09, the user's steer: a **spectral view**, swapped in by the *Spectrum* button next to ▶ and ■. One Hann-windowed FFT per pixel column over the view (the FFT length follows the zoom, 256 to 2048 samples), magnitudes in dB below the loudest bin in view down to −90 dB, rows on a log-frequency axis from 30 Hz to half the sample rate with 100 Hz / 1 kHz / 10 kHz marked; the panel ground through the accent to amber and white. It is computed from the samples kept in memory, so it costs the same for a 4-s vowel and a 2-minute ambience (22 ms a raster) and is not offered for files beyond three minutes. Zoom, markers, the windows and the playhead work the same; the choice is remembered.)*
...
*(2026-09-09, the user's steer: the **columns are configurable and remembered**. A header section drags to a new place; a right-click on one opens a menu — *Replace “…” with* a hidden column (it takes that place, the clicked one goes), a checklist to show or hide any column (never the last one), *Reset columns*. Similarity and Match stay the scores' to show and hide. Two **fixed columns** open every row (the user's steer, the same day): the tree expander and the anchor circle — pinned at the left, unmovable, unresizable, unsortable, no filter; the expander column's header cell shows ▸ or ▾ and a click on it expands or collapses every sample's sections (the *Sections* button of an hour earlier is gone again). A section row's label sits in the first movable column shown. The header's state — order, widths, hidden columns, the sort — is saved on every change and restored at the next start, sort included.)*
...
*(Phase 11, 2026-09-13 — the **Correct** section under the difference bars, `corrections.py` behind it: the selected sample's **class** and **type** as segmented controls, a note beside each saying whose call it is ("CLAP's call: Vocal 70 %" / "yours — CLAP said …" / "the rule's call from the descriptors") and a ↺ back to the machine's call — a class reset is the best of the stored four scores under the tab's confidence threshold, a type reset the Phase 2 rule over the stored descriptors under the tab's one-shot cap, no audio either way; **My tags**, the curated layer as chips (the text searches, the × takes one off) with a box to add one (completions from every curated tag); and **Suggested**, CLAP's chips the sample does not carry yet, a click promoting one (§8's flow). Each correction is one UPDATE and the row refreshes in place — no job, no reload — and is protected by its facet's own flag, so correcting the class leaves the type free to re-analyse and vice versa. With several rows selected, an *Apply to all N selected rows* box sends a class, a type or a tag to every one. A type correction changes what the next Recompute does — node S reads the index — but nothing runs on its own (§9.6).)*
...
*(Phase 12, 2026-09-13, after the user's first full run of the library — ten silent minutes of scanning, an hour-plus of analysis and segmentation, a 16-hour projection for CLAP. **Feedback:** the scanner walks first, then reads headers on threads with a progress line every 5,000 files; every stage's progress line carries an ETA from its running rate; the embedding stage opens by counting the whole-file clips and the segments it is about to run through CLAP, since a segment costs the same fixed 10-s pass as a file. **Speed:** *Recompute attributes* runs analysis and segmentation as **one pass per file** (`describe.py`, nodes B + C + S + T + C2 together) — one decode and one set of frame features serve the descriptors, the structural type, the transients and every segment's descriptors, where the two stages in sequence decoded and HPSS-split every file twice. Measured 1.84× on 1,616 real files with 8 workers, identical segments; more worker processes measured slower than 8, so the cap stays. A window's descriptors are the parent's frames inside it, by one definition everywhere (`analysis.describe_window`). The stand-alone `crate-analyze` / `crate-segment` stay for the CLI. `.docs/phase12_profile.md` has the numbers.)*

*(Phase 12, 2026-09-13, the user's steer — "make the recompute a background job with progress feedback; normal use of the application should be available in the meantime; a job queue where recompute tasks and user tasks mix". **The job queue** (`jobqueue.py`): one worker thread with one connection — the index keeps its single writer — and two kinds of job. A **batch** (a recompute, a rescan, a library-wide layout, a caption run, a folder removal) is long and calls a hook after each file it commits; between two files the worker runs every **interactive** job waiting — a segment save or delete, an anchored recompute, an anchored map placement, one caption — on the same thread and connection, then carries on. "A job is running" is gone: a user's small write waits at most one file's work. One batch at a time; Stop lands after its current file. A recompute takes the **list's order**, top to bottom as sorted and filtered when Run is pressed, so what the user is looking at is done first and what the filter hides comes last; "new/changed only" skips what is done. The batch reports **progress** — stage, done, total, ETA — to a bar at the right of the status bar, and hands back the ids it finished every two seconds so the window **refreshes those rows and their sections in place** (`SampleTreeModel.update_sample`): the list fills in while the run goes on, without a reload and without losing the selection. Every job's end still reloads the list. Removing a folder waits for the queue to be quiet. Weighed and not chosen: the recompute as a separate OS process polling the index — crash isolation, at the price of a second writer process with no way to interleave a user's write between files by priority, and a channel to build for progress, order, Stop and the plan chain; the queue's shape keeps that a contained change if the window ever stutters under a real run.)*
...
**Rust** stays a reserved, not scheduled, option — introduced only if Phase 12 (Scale & Polish) profiling identifies one specific hot path where even vectorized Python is the bottleneck relative to model-inference cost, via a narrow same-process extension (PyO3/`maturin`), not a rewrite. *(Phase 12's pass, 2026-09-13, `.docs/phase12_profile.md`: against the real 84k-sample / 394k-item index the one hot path was a per-row Python parser in the feature-table load — 1.17 million `json.loads` calls — and vectorising it took the load from 12.6 s to 4 s and its arrays from 943 MB to ~130 MB. Nothing that remains is Python-bound. Rust stays reserved.)*
...
**Phase 11 — Correction Workflow.** Manual classification/tag overrides, protection from automatic overwrite. *(Built 2026-09-13, pulled ahead of Phase 10 on the user's steer — see the §9.5 note: `corrections.py`, schema v11 with one protection flag per facet and the §8 `tags` / `sample_tags` layer, the Correct section on the Attributes tab, a "My tags" list column.)*
**Phase 12 — Scale & Polish Hardening.** Cache eviction, incremental re-scan, corrupt/exotic-file handling, profiling pass (informs whether Rust is ever justified, §10). *(Built 2026-09-13 — `.docs/phase12_profile.md`: the pass over the real index found one hot path, the feature-table load, now column-wise with compact vectors (§10's verdict: no Rust); the list and the scanner's walk measured fine at 84k samples / 310k sections, so no lazier section model; render-cache eviction under a limit on the Library panel (§6.5); AppleDouble `._` files skipped at scan and header-less rows shown as `unreadable` in the list's Type column. A second round the same day, after the user's first full run: scan progress and threaded header reads, ETAs on every stage, the embedding stage's opening count, and analysis + segmentation as one pass per file — 1.84× measured; see the §9.6 note.)*
