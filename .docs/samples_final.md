...
*(2026-09-08, the user's steer — "the Attributes tab is quite crammed": the right panel has three tabs. **Attributes** is about the selected sample — its per-axis difference from the anchor, its CLAP chips (a click hands the tag to the Search tab), its Qwen2-Audio caption with a *Caption this sample* button, its four CLAP scores, its embedding as stripes, its segments, and the weight bars. **Search** (`search.py`) holds the CLAP text search box and every filter: type, minimum CLAP scores, length, tempo, and the anchor-distance ranges. **Recompute** is §9.6.)*
