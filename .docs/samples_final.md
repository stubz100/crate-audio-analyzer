...
- **View switch** (`Map`/`List`), always visible. *(2026-09-08, the user's steer: the header runs across the whole window above the list and the tabs — the List and Map buttons stacked at the left with room for a third, then the selected sample's ten best **CLAP tag scores as vertical bars** (cosine ×100 on a 100 scale, the tag under each; a click searches for it, like a chip), and its **CLAP embedding** as a thin strip underneath, the anchor's below it. The quick-filter box that used to sit here is gone: filtering lives in the list's column headers, §9.4.)*
...
*(2026-09-08, the user's steer — "the Attributes tab is quite crammed": the right panel has three tabs. **Attributes** is about the selected sample — its per-axis difference from the anchor, its CLAP chips (a click hands the tag to the Search tab), its Qwen2-Audio caption with a *Caption this sample* button, its four CLAP scores, its embedding as stripes, its segments, and the weight bars. **Search** (`search.py`) holds the CLAP text search box and every filter: type, minimum CLAP scores, length, tempo, and the anchor-distance ranges — since later that day only the minimum CLAP scores and the anchor-distance ranges, as type, length and tempo filter from the list's column headers (§9.4). **Recompute** is §9.6. Later the same day, on the user's steer, the *CLAP scores* box on Attributes shows the **zero-shot tag scores** — cosine ×100 against each tag prompt, best first, ten of the thirty-two, the numbers behind the chips — instead of the four prompt-set percentages, which now feed only the Search tab's minimum-score filter; every tag is stored per sample since (`EmbedSettings.top_k_tags` 0 = all; `crate-embed --reclassify` fills an older index).)*
