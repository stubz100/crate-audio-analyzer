...
The right half of the window is the list (or the map) top to bottom. The left half starts with a header holding the List / Map switch, the selected sample's ten best CLAP tag scores as bars (click one to search for it) and its CLAP embedding as a strip. Click any column header of the list to sort it or filter on it: a text for File, Folder and Tags, a checklist for Type and Key, a min/max for Length, BPM, Hits, Similarity and Match; a filtered column shows a dot.
