...
The right half of the window is the list (or the map) top to bottom. The left half starts with a header holding the List / Map switch, the selected sample's ten best CLAP tag scores as bars (click one to search for it) and its CLAP embedding as a strip. The list's columns run Folder, File, Caption (the Qwen2-Audio sentence), Length, Type, BPM, Key, Tags, Hits; every section of a sample sits under it as a child row, in time order, or by descending similarity once anchored. The first two columns are fixed: the expander (click its header cell to expand or collapse every sample's sections) and the anchor circle. Drag any other column header to move it, right-click one to replace it with a hidden column, show or hide columns, or reset them; the arrangement, the widths and the sort are remembered, as are the window's size, the open tab and the List / Map choice. Click any column header to sort it or filter on it: a text for Folder, File, Caption and Tags, a checklist for Type and Key, a min/max for Length, BPM, Hits, Similarity and Match; a filtered column shows a dot.

Then, on the **Search** tab: type what you are after ("footsteps on gravel", "sword clash") and press Search — a segment that matches better than its parent shows as an indented *hit* row you can preview and drag, and a long file whose best part is one of its 10-s CLAP windows shows an indented *window* row the same way, so a search can land at minute seven of an ambience. Click the circle at the start of any row (a hit row too): that sample becomes the anchor and the whole list is ranked against it at once, anchor on top, with a Similarity column; the distance ranges under Filters then cut the list down per axis (the Search tab keeps those and the CLAP minimum scores; everything with a column filters from the column's header). Move the weight bars (Search tab, under the filters) and the list re-ranks as you release them; a click on the filled circle clears the anchor and leaves the order alone. **Recompute map layout** (same tab, needs `uv sync --extra map`) fits the map; the **Map** button switches the list for it — click a point to select and preview, wheel to zoom, drag to pan, right-click to fit.

Under that header, the waveform panel shows the selected sample's Qwen2-Audio caption (or “—”) with its Caption/Recaption button, then the waveform with its segments as begin/end markers, the measured attack/decay envelope and the playhead; the wheel zooms about the cursor, Shift+wheel or the scrollbar pans, right-click fits; **Spectrum** next to the transport buttons swaps the waveform for a log-frequency spectrogram of the same view. The audio is read on a thread, so the preview never waits for the drawing. Drag a marker to move it, drag on the waveform to draw a new segment; nothing is written until **Save segment** (a moved automatic segment becomes a manual one, which no recompute touches), **Discard** or Esc drops the unsaved markers, **Delete segment** removes the selected one after asking; click inside a segment to preview it; ▶, ■ and *Auto-play on select* sit in the same row as the segment buttons; the Attributes / Search / Recompute tabs sit below, the split with the waveform half and half; the Attributes tab starts with the per-axis difference between the selected sample and the anchor. Analysis and segmentation fan out to worker processes (Recompute tab → Worker processes, or `--workers N`). The Rhythmic / Melodic / Vocal / Other columns are CLAP's own numbers: how well the sample matches each of four prompt sets, as percentages that sum to 100 (the prompts are in the tooltips on the Attributes tab). The map is coloured by the current ranking or search score, one colour otherwise. The window uses a dark theme (`theme.py`). **Captions** (a step under Run on the Recompute tab, off by default, a batch of files per Run so a folder gets done in sittings; **Caption this sample** on the Attributes tab for one file; `crate-caption`) write one Qwen2-Audio sentence per sample and show it on the Attributes tab — about 10 s per file; the Phase 5 spike (`.docs/phase5_spike.md`) found the model's encoder latent no better than CLAP for telling voices apart, so there is no vocal axis.
