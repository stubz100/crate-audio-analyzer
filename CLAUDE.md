...
- **Nothing expensive runs automatically.** Map layout and attribute/embedding recomputation are always explicit, on-demand user actions (spec §9.6) — never on a timer, a slider drag, or in the background. Ranking is the measured exception (2026-09-08, the user's steer): the ⚓ on a list row ranks at once and the weight bars re-rank on release, because a full ranking is one vectorised pass of ~25 ms; if that ever stops being true, it goes back behind a button.
